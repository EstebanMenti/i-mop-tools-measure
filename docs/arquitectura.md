...
| **D3** — Una medición direccional se hace de a una por vez (no todas en paralelo). El nodo iniciador se conecta una sola vez por grupo (`open_initiator`) y se reusa contra todos sus respondedores en secuencia (`run_directed_measurement` × N-1, `close_initiator` al final del grupo, ver `ranging/campaign.py`); el respondedor sí se conecta y desconecta en cada dirección. | El firmware/BLE del repo hermano documenta un límite duro de conexiones BLE simultáneas (`max_concurrent_connections` en `[ble_timeouts]`, ~5-7 según el bridge) — medir de a una evita saturar el enlace y simplifica el manejo de errores por nodo. Reusar la conexión del iniciador quedó explícitamente pospuesto en la version original de esta decisión (con 2 nodos el costo extra de reconectar era mínimo); con 4 nodos activos se midió contra hardware real que conectar por BLE cuesta ~10s (con ~15-20 dispositivos BLE alrededor en el scan, 2026-09-08) — el ítem más caro de una medición — y se paga una vez por cada una de las `N-1` direcciones donde ese nodo es iniciador, así que se implementó el reuso: ~19% menos tiempo total de campaña en la práctica, sin reducir muestras ni tolerancias, reusando únicamente mecanismos de resiliencia ya existentes (reconexión automática en cada comando, ver transport/ble_link.py `_ensure_connected`). No se aplicó al respondedor porque forzaría reordenar el enfoque de la campaña por respondedor en vez de por iniciador, sin poder combinar ambos a la vez dado que solo corre una sesión de ranging genuina por vez. Paralelizar mediciones distintas (más de una sesión activa a la vez) queda para una fase posterior. |

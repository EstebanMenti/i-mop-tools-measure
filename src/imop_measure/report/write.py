...
from imop_measure.report.build import DEFAULT_REVIEW_THRESHOLD_CM, DEFAULT_TOLERANCE_CM, summarize
...
def write_reports(
    ...
    *,
    ...
    `samples`/`tolerance_cm`/`review_threshold_cm` son solo para mostrar
    en el encabezado del reporte (los parámetros ya se aplicaron antes,
    en `report.build.build_results`) — si se omite `samples`, se toma de
    la primera fila (`n_samples_requested`).
    ...
    payload = {
        ...
        "parametros": {
            ...
            "umbral_revision_cm": review_threshold_cm,
        },
        "resumen": resumen,
...
def _render_markdown(
    ...
    *,
    ...
    lines.append(
        f"**Criterios:** tolerancia ±{tolerance_cm:.1f} cm para PASS/FAIL · "
        f"umbral de revisión ±{review_threshold_cm:.0f} cm"
    )
    ...
    a_revisar = [r for r in results if r.estado != "PASS" or r.necesita_revision]
    if a_revisar:
...
def _render_resumen_table(resumen: dict[str, int]) -> list[str]:
    return [
        ...
        "|---|---|---|---|---|",
        f"| {resumen['total']} | {resumen['pass']} | {resumen['fail']} | "
        f"{resumen['error']} | {resumen['revisar']} |",
    ]
...
def _render_revision_item(result: PairResult) -> str:
    ...
    revisar_nota = " · diferencia mayor al umbral de revisión" if result.necesita_revision else ""
    return (
